...
public class Consumer3 {
    public static void main(String[] args) throws Exception {
        ...
        channel.queueDeclare("zx_queue_t3", true, false, false, null);

        //queue:queue名称
        //exchange:exchange名称
        //routingKey:路由键;用来绑定queue和exchange
        //queueBind(String queue, String exchange, String routingKey)
        channel.queueBind("zx_queue_t3","zx_topic","zx.queue");

        //消息消费
        //queue:绑定队列名称
        //autoAck：是否自动ack，如果不自动ack，需要使用channel.ack、channel.nack、channel.basicReject 进行消息应答
        //basicConsume(String queue, boolean autoAck, Consumer callback)
        channel.basicConsume("zx_queue_t3",false,new DefaultConsumer(channel){
            @Override
